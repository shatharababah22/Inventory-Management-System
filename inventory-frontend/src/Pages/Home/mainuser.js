      ...
      const fetchData = async () => {
        try {
          if (!authTokenUser) {
            ...
            const headers = { Authorization: `Bearer ${authTokenUser}` };
            const recentResponse = await axios.get("http://127.0.0.1:8000/api/products?_sort=createdAt&_order=desc&_limit=5", { headers });
            const expiredResponse = await axios.get("http://127.0.0.1:8000/api/expire_products", { headers });
            setRecentlyAddedProducts(recentResponse.data);
